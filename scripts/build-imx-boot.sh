...
if [ ! -d imx-seco-5.8.7.1 ]; then
    wget -nc https://www.nxp.com/lgfiles/NMG/MAD/YOCTO/imx-seco-5.8.7.1.bin
    chmod u+x imx-seco-5.8.7.1.bin
    ./imx-seco-5.8.7.1.bin --auto-accept --force
    rm -f imx-seco-5.8.7.1.bin
fi
...
if git apply --check ../../patches/imx-atf/0001-Revert-Add-NXP-s-SoCs-partition-reboot-support.patch > /dev/null 2>&1; then
    git apply ../../patches/imx-atf/0001-Revert-Add-NXP-s-SoCs-partition-reboot-support.patch
fi
if git apply --check ../../patches/imx-atf/0002-imx8m-hab.c-work-around-gcc-12.1-false-positives.patch > /dev/null 2>&1; then
    git apply ../../patches/imx-atf/0002-imx8m-hab.c-work-around-gcc-12.1-false-positives.patch
fi
...
echo "Finished build-imx-boot.sh"
